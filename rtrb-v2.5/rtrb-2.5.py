        ...
        if req.status_code == 204:
            ...
            if rcounter % 6 == 0:
                ...
                if result.returncode == 0:
                    ...
                    # Print the results
                    print_ts(f"\033[1;44;37m Internet \033[0m   Download: {download_speed} Mbps / Upload: {upload_speed} Mbps / Ping: {ping} ms\033[m")
                    log_message(f"[Internet]   Download: {download_speed} Mbps / Upload: {upload_speed} Mbps / Ping: {ping}")
                    #print(f"Download: {download_speed} Mbps")
                    #print(f"Upload: {upload_speed} Mbps")
                    #print(f"Ping: {ping} ms")
                else:
                    #print("Error running speedtest-cli:", result.stderr)
                    print_ts("\033[1;44;37m Internet \033[m\033[31m   Error running speedtest\033[m")
                    log_message("[Internet]   Error running speedtest")
                time.sleep(.1)
                # Get cpu temp
                ...
                log_message(f"[CPU     ]   Usage: {cpu_usage}% / Temp: {cputemp}°c / Freq: {cpu_speed} MHz / Load: {load15} / Memory Usage: {memory_info.percent}%")
                time.sleep(.1)
            rcounter += 1
            icounter += 1
            return True
